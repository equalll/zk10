    ...
    def __onNewValue(self, dataSeries, dateTime, value):
        ...
        if value is not None:
            ...
            if sma is not None:
                ...
                dnum=self.__numStdDev
                upperValue = sma + stdDev * dnum
                lowerValue = sma + stdDev * dnum * -1
                #dprice=bars[self.__instrument].getPrice();
                nss=dateTime.strftime('%Y-%m-%d')
                print(nss,'boll,%.2f,%.4f,%d,%.2f,%.2f' %(sma,stdDev,dnum,upperValue,lowerValue))
