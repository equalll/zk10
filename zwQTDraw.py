...
def dr_quant3x_init(qx,w,h):
    ...
    left, width = 0, 1.0
    p1box = [left, 0.7, width, 0.2]# [左, 下, 宽, 高] 规定的矩形区域 （全部是0~1之间的数，表示比例）
    p2box = [left, 0.2, width, 0.5]
    p3box = [left, 0.0, width, 0.2]
    #--set qx.size&pos
    qx.pltTop = fig.add_axes(p1box);qx.pltTop2 = qx.pltTop.twinx()
    qx.pltMid = fig.add_axes(p2box);qx.pltMid2 = qx.pltMid.twinx()
    qx.pltBot = fig.add_axes(p3box);qx.pltBot2 = qx.pltBot.twinx()
    #--set label,tick,...
...
def dr_quant3x00(qx,ktop2,kbot,kmidlst,midSgn0=''):
    ...
    for x5 in kmidlst:
        ...
        for xc in range(xn9-1):
            ...
            css=ksgn;
            if midSgn0!='':
                if midSgn0=='<xcod>':css=xcod+'_'+css;
                else:css=midSgn0+'_'+css;
            x10.append(css);
